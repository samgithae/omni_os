...
function handleTierFilter(tier: string) {
  if (tier === activeTierFilter.value) {
    ...
    router.get(qs ? `/leads?${qs}` : '/leads', {}, { preserveScroll: true, preserveState: true, replace: true })
  } else {
    activeTierFilter.value = tier
    const params = new URLSearchParams(window.location.search)
    params.set('tier', tier)
    params.delete('page')
    router.get(`/leads?${params.toString()}`, {}, { preserveScroll: true, preserveState: true, replace: true })
  }
}
...
<template>
  ...
  <div class="pb-8">
    ...
    <div class="flex items-center gap-3 border-b border-gray-100 px-4 py-1.5 text-[10px] font-medium uppercase tracking-wider text-gray-400">
      ...
      <button class="flex w-10 items-center gap-0.5 cursor-pointer hover:text-gray-600" @click="sortBy('score')">
        Score
        <ArrowUpDown class="h-3 w-3" :class="{ 'text-blue-500': sortColumn === 'score' }" />
      </button>
      <div class="flex-1">
        <button class="flex items-center gap-0.5 cursor-pointer hover:text-gray-600" @click="sortBy('company_name')">
          Company
          <ArrowUpDown class="h-3 w-3" :class="{ 'text-blue-500': sortColumn === 'company_name' }" />
        </button>
      </div>
      <button class="flex w-20 items-center justify-end gap-0.5 cursor-pointer hover:text-gray-600" @click="sortBy('status')">
        Status
        <ArrowUpDown class="h-3 w-3" :class="{ 'text-blue-500': sortColumn === 'status' }" />
      </button>
