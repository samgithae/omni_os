...
<template>
  <div class="flex items-center gap-0 border-b border-gray-200 px-4 py-2.5 text-xs">
    ...
    <div class="flex items-center gap-1 px-3 py-1">
      <span class="text-lg font-semibold leading-none text-gray-900">{{ stats.total.toLocaleString() }}</span>
    ...
    <!-- Avg Score -->
    <div class="flex items-center gap-1 px-3 py-1">
      <span class="text-lg font-semibold leading-none text-indigo-600">{{ stats.avg_score }}</span>
    ...
    <!-- Tier pills -->
    <button
      v-for="tier in ['hot', 'warm', 'moderate', 'cold', 'frigid'] as const"
      :key="tier"
      class="flex cursor-pointer items-center gap-1 px-3 py-1 transition-colors hover:bg-gray-50 rounded"
      :class="{ [tierMeta[tier].bg]: activeTier === tier }"
      @click="clickStat(tier)"
    >
      <span class="text-lg font-semibold leading-none" :class="tierMeta[tier].color">
    ...
    <!-- With Email -->
    <div class="flex items-center gap-1 px-3 py-1">
      <span class="text-lg font-semibold leading-none text-emerald-600">{{ stats.with_email }}</span>
      <span class="text-gray-500">With Email</span>
    </div>

    <!-- Enriched -->
    <div class="flex items-center gap-1 px-3 py-1">
      <span class="text-lg font-semibold leading-none text-blue-600">{{ stats.enriched }}</span>
