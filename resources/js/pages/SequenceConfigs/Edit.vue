...
function submit() {
  ...
  const url = isEditing ? `/sequence-configs/${props.config!.id}` : '/sequence-configs'
...
<template>
  ...
  <div>
    <div class="flex items-center gap-2 border-b border-gray-200 px-4 py-3">
      <Link href="/sequence-configs" class="rounded p-1 text-gray-400 hover:text-gray-600">
        <ArrowLeft class="h-4 w-4" />
      </Link>
      <FileText class="h-4 w-4 text-gray-400" />
      <h1 class="text-sm font-semibold text-gray-900">
        {{ isEditing ? `Edit: ${config?.brand?.name || ''} / ${config?.segment || ''}` : 'New Sequence Config' }}
      </h1>
    </div>
    ...
    <form @submit.prevent="submit" class="mx-auto max-w-3xl space-y-6 p-6">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="mb-1 block text-xs font-medium text-gray-700">Brand</label>
          <select v-model="form.brand_id" class="w-full rounded border-gray-200 text-sm" :class="{ 'border-red-400': errors.brand_id }">
            <option v-for="b in brands" :key="b.id" :value="b.id">{{ b.name }}</option>
          </select>
          <p v-if="errors.brand_id" class="mt-1 text-xs text-red-500">{{ errors.brand_id }}</p>
        </div>
        <div>
          <label class="mb-1 block text-xs font-medium text-gray-700">Segment</label>
          <select v-model="form.segment" class="w-full rounded border-gray-200 text-sm">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="mb-1 block text-xs font-medium text-gray-700">Number of Emails</label>
          <input type="number" v-model.number="form.sequence_steps" min="1" max="10" class="w-full rounded border-gray-200 text-sm" />
        </div>
      ...
      <div>
        <label class="mb-1 block text-xs font-medium text-gray-700">
          ...
          <span class="text-gray-400 font-normal"> — Full instructions Hermes uses to generate emails</span>
        </label>
        <textarea
        ...
        ></textarea>
        <p v-if="errors.prompt_text" class="mt-1 text-xs text-red-500">{{ errors.prompt_text }}</p>
      </div>

      <!-- Submit -->
      <div class="flex items-center justify-end gap-2 border-t border-gray-100 pt-4">
        <Link href="/sequence-configs" class="rounded px-3 py-1.5 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
          Cancel
        </Link>
