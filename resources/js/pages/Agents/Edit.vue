...
function generateToken() {
    if (
        !confirm(
            'Generate a new token? The current token will be invalidated immediately.',
        )
    )
        return;
...
function uploadDocument() {
    ...
    fetch(`/agents/${props.agent.id}/documents`, { method: 'POST', body: fd })
        ...
        .then((doc) => {
            ...
            (
                document.getElementById('doc-file-input') as HTMLInputElement
            ).value = '';
        })
        .catch(() => {
...
<template>
    ...
    <div class="mx-auto max-w-2xl pb-16">
        ...
        <Transition name="fade">
            ...
            >
                <div class="w-full max-w-lg rounded-xl bg-white p-6 shadow-2xl">
                    ...
                    <div v-else>
                        <div
                            class="mb-2 rounded-lg bg-amber-50 px-4 py-3 text-xs text-amber-800"
                        >
                            <strong
                                >Store this now — it will not be shown
                                again.</strong
                            >
                        ...
                        <div class="relative mb-4">
                            <pre
                                class="overflow-x-auto rounded-lg bg-gray-50 px-4 py-3 font-mono text-xs text-gray-800"
                                >{{ newToken }}</pre
                            >
                        </div>
        ...
        <form @submit.prevent="submit" class="space-y-4 px-4 pt-4">
            ...
            <div>
                ...
                <div class="flex items-center gap-4">
                    <div
                        class="h-14 w-14 shrink-0 overflow-hidden rounded-full bg-gray-200"
                    >
                        <img
                    ...
                    <div class="flex items-center gap-2">
                        <label
                            class="cursor-pointer rounded-lg bg-white px-3 py-1.5 text-xs font-medium text-blue-600 ring-1 ring-gray-200 ring-inset hover:bg-blue-50"
                        >
        ...
        <div class="mt-6 border-t border-gray-200 px-4 pt-4">
            <h2
                class="mb-3 flex items-center gap-2 text-xs font-semibold text-gray-700"
            >
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs text-gray-600">
                            Token:
                            <span
                                v-if="agent.token_last_four"
                                class="font-mono font-medium text-gray-900"
                                >…{{ agent.token_last_four }}</span
                            >
                            <span v-else class="text-gray-400 italic"
                                >Not generated yet</span
                            >
                        </p>
                        <p
        ...
        <div class="mt-6 border-t border-gray-200 px-4 pt-4">
            ...
            <p v-else class="mb-3 text-[11px] text-gray-400 italic">
            ...
            <div class="flex items-end gap-2">
                ...
                <button
                    ...
                    :disabled="
                        uploadingDoc || !newDocLabel.trim() || !newDocFile
                    "
            ...
            <p v-if="docErrors" class="mt-1 text-[10px] text-red-500">
                {{ docErrors }}
            </p>
        </div>
    </div>
