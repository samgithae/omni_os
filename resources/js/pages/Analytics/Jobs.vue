...
async function runJob(job: Job) {
    ...
    try {
        const res = await fetch(
            `/analytics/jobs/${encodeURIComponent(job.name)}/run`,
            {
                method: 'POST',
...
async function fetchHistory(jobName: string) {
    ...
    try {
        const res = await fetch(
            `/analytics/jobs/history?${params.toString()}`,
            {
                credentials: 'same-origin',
                headers: {
                    Accept: 'application/json',
    ...
    <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-muted-foreground">
                    Cron job monitoring &amp; run history
                </p>
            </div>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
            <button
                ...
                :class="
                    activeGroup === g || (!activeGroup && g === 'all')
            ...
            >
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-2">
                            <span
                                ...
                                >{{ groupLabels[job.group] || job.group }}</span
                            >
                            <span
                                class="rounded-full bg-gray-100 px-2 py-0.5 font-mono text-[10px] text-gray-600"
                                >{{ job.schedule_label }}</span
                            >
                        </div>
                    ...
                    <div class="ml-4 shrink-0 text-right">
                        ...
                        <div
                            v-if="job.last_run?.duration_ms"
                            class="mt-0.5 text-[10px] text-muted-foreground"
                        >
                            Duration:
                            {{ formatDuration(job.last_run.duration_ms) }}
                        </div>
                    </div>
                </div>
                ...
                <!-- Quick stats row -->
                <div
                    class="mt-3 flex items-center gap-4 border-t pt-2 text-[10px] text-muted-foreground"
                >
                    <span
                        >Total:
                        <strong>{{ job.stats.total_runs }}</strong></span
                    >
                    <span
                        >24h: <strong>{{ job.stats.runs_24h }}</strong></span
                    >
                    <span
                        v-if="job.stats.success_24h > 0"
                        class="text-emerald-600"
                        >✅ <strong>{{ job.stats.success_24h }}</strong></span
                    >
                    <span v-if="job.stats.failed_24h > 0" class="text-red-600"
                        >❌ <strong>{{ job.stats.failed_24h }}</strong></span
                    >
                    <span
                        v-if="job.stats.running_24h > 0"
                        class="text-amber-600"
                        >⏳ <strong>{{ job.stats.running_24h }}</strong></span
                    >
                    <button
                    ...
                    <button
                        class="inline-flex items-center gap-1 rounded bg-blue-600 px-2 py-1 text-[10px] font-medium text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-40"
                    ...
                    <div
                        v-if="runResults[job.name] && runningJob !== job.name"
                        class="ml-2 text-xs font-medium"
                        :class="
                            runResults[job.name].startsWith('✅')
                                ? 'text-emerald-600'
                                : 'text-red-600'
                        "
                    >
                ...
                <div v-if="selectedJob === job.name" class="mt-3 border-t pt-3">
                    ...
                    <div v-else class="max-h-60 overflow-y-auto">
                        <table class="w-full text-xs">
                            <thead>
                                <tr
                                    class="border-b text-left text-[10px] tracking-wider text-muted-foreground uppercase"
                                >
                                ...
                                >
                                    ...
                                    <td class="px-2 py-1.5">
                                        <span
                                            ...
                                            class="inline-flex items-center gap-0.5 text-emerald-600"
