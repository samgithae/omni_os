...
function toggleInlineEdit(id: number) {
    editingInline.value = {
        ...editingInline.value,
        [id]: !editingInline.value[id],
    };
}

function saveInline(item: ScheduleData) {
    router.put(
        `/sequence-schedules/${item.id}`,
        {
...
function goToPage(page: number) {
    ...
    router.get(`/sequence-schedules?${params.toString()}`, {
        preserveScroll: true,
        preserveState: true,
        replace: true,
    });
    ...
    <div>
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody>
                    ...
                    >
                        ...
                        <td class="px-4 py-2.5">
                            <input
                                v-if="editingInline[s.id]"
                                type="number"
                                v-model.number="s.days_after_previous"
                                min="0"
                                class="w-16 rounded border-gray-200 text-xs"
                            />
                            <span v-else class="text-gray-600">{{
                                s.days_after_previous
                            }}</span>
                        </td>
                        <td
                            class="max-w-[200px] truncate px-4 py-2.5 text-gray-500"
                        >
                            <input
                                v-if="editingInline[s.id]"
                                v-model="s.purpose"
                                class="w-full rounded border-gray-200 text-xs"
                            />
                        ...
                        <td class="px-4 py-2.5">
                            <div class="flex items-center gap-1">
                                <button
                                    ...
                                    title="Edit inline"
                                    @click="toggleInlineEdit(s.id)"
                                >
                                    <Pencil class="h-3.5 w-3.5" />
                                </button>
                                <template v-if="editingInline[s.id]">
                                    <button
                                        class="rounded bg-blue-600 px-2 py-0.5 text-[10px] font-medium text-white"
                                        @click="saveInline(s)"
                                    >
                                        Save
                                    </button>
                                    <button
                                        class="rounded px-2 py-0.5 text-[10px] font-medium text-gray-600 ring-1 ring-gray-300 ring-inset"
                                        @click="toggleInlineEdit(s.id)"
                                    >
                                        Cancel
                                    </button>
        ...
        >
            <span class="text-xs text-gray-500"
                ...
                {{ schedules.last_page }}</span
            >
            <div class="flex items-center gap-1">
                <button
                    v-for="page in schedules.last_page"
                    :key="page"
