...
function confirmDelete(id: number) {
    if (confirm('Remove this suppression?'))
        router.delete(`/suppressions/${id}`, {
            preserveScroll: true,
            preserveState: true,
        });
}
...
function applyFilter(key: string, value: string) {
    ...
    params.delete('page');
    router.get(`/suppressions?${params.toString()}`, {
        preserveScroll: true,
        preserveState: true,
        replace: true,
    });
    ...
    <div>
        ...
        <div v-else class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                ...
                <tbody>
                    <tr
                        v-for="s in suppressions.data"
                        :key="s.id"
                        class="border-b border-gray-50 hover:bg-gray-50/50"
                    >
                        <td class="px-4 py-2.5 font-mono text-xs text-gray-800">
                            {{ s.email }}
                        </td>
                        ...
                        <td class="px-4 py-2.5">
                            <span
                                ...
                                :class="reasonBadge(s.reason)"
                                >{{ s.reason }}</span
                            >
                        </td>
                        <td
                            class="max-w-[200px] truncate px-4 py-2.5 text-xs text-gray-400"
                        >
                            {{ s.notes || '-' }}
                        </td>
        ...
        <div
            v-if="suppressions.last_page > 1"
            class="flex items-center justify-between border-t border-gray-100 px-4 py-3"
        >
            <span class="text-xs text-gray-500"
                >Page {{ suppressions.current_page }} of
                {{ suppressions.last_page }}</span
            >
            <div class="flex items-center gap-1">
                <button
                    v-for="page in suppressions.last_page"
                    :key="page"
                    class="rounded px-2.5 py-1 text-xs font-medium"
                    :class="{
                        'bg-blue-600 text-white':
                            page === suppressions.current_page,
