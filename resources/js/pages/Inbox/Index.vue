...
function sendReply() {
    ...
    router.post(
        `/inbox/${selectedLeadId.value}/reply`,
        {
        ...
        <div class="flex w-[360px] shrink-0 flex-col border-r border-gray-200">
            ...
            >
                ...
                <span class="text-xs text-gray-500"
                    >{{ replies.total }} total</span
                >
            </div>

            <!-- Filter bar -->
            <div
                class="flex flex-wrap items-center gap-1.5 border-b border-gray-200 px-3 py-2"
            >
                <select
                ...
                <div class="relative min-w-[100px] flex-1">
                    <Search
                        class="pointer-events-none absolute top-1/2 left-1.5 h-3 w-3 -translate-y-1/2 text-gray-400"
                    />
                    <input
                        v-model="searchInput"
                        type="text"
                        placeholder="Search..."
                        class="w-full rounded-md border border-gray-200 py-1 pr-2 pl-6 text-xs outline-none placeholder:text-gray-400 focus:border-blue-300"
                    />
                </div>
                    ...
                    >
                        <div class="flex items-start gap-2">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p
                                    v-if="reply.subject"
                                    class="mt-0.5 truncate text-xs text-gray-600"
                                >
                                    {{ reply.subject }}
                                </p>
                    ...
                    >
                        ...
                        <div class="flex gap-1">
                            <button
                                ...
                                :disabled="!link.url"
                                @click="
                                    link.url && goToPage(Number(link.label))
                                "
                                v-html="link.label"
                            />
                        </div>
        ...
        <div class="flex flex-1 flex-col overflow-hidden">
            ...
            >
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-400">
                        Replies from leads will appear here with full thread
                        history
                    </p>
                </div>
            </div>
            ...
            <template v-else>
                <!-- Conversation header -->
                <div
                    class="flex items-center gap-2 border-b border-gray-200 px-4 py-3"
                >
                    <button
                        @click="selectedLeadId = null"
                    ...
                    <div class="flex-1">
                        <h2 class="text-sm font-semibold text-gray-900">
                            {{ leadContext?.company_name || 'Loading...' }}
                        </h2>
                        <p class="text-xs text-gray-500">
                            {{ leadContext?.email }} &middot;
                            {{ leadContext?.segment }} &middot; Score:
                            {{ leadContext?.score }}
                        </p>
                    </div>
                    <a
                        v-if="leadContext"
                        :href="`/admin/leads/${leadContext.id}`"
                        ...
                        >
                            ...
                            >
                                ...
                                <template v-if="item.type === 'email'">
                                    <div
                                        class="mb-1 text-[10px] tracking-wider uppercase opacity-60"
                                    >
                                        Sent email &middot; Step
                                        {{ item.sequence_step }}
                                        <span v-if="item.opened_at">
                                            &middot; Opened</span
                                        >
                                    </div>
                                    ...
                                    <div
                                        ...
                                        v-html="bodySnippet(item.body, 200)"
                                    />
                                </template>
                                ...
                                <template v-else>
                                    ...
                                    <p
                                        v-if="item.classification_summary"
                                        class="mt-2 text-xs italic opacity-60"
                                    >
                ...
                <div class="border-t border-gray-200 px-4 py-3">
                    <div
                        v-if="replyError"
                        class="mb-2 rounded-md bg-red-50 px-3 py-1.5 text-xs text-red-600"
                    >
                        {{ replyError }}
                    </div>
                    <div
                        v-if="replySuccess"
                        class="mb-2 rounded-md bg-green-50 px-3 py-1.5 text-xs text-green-600"
                    >
                        {{ replySuccess }}
                    </div>

                    <div
                        v-if="leadContext?.is_suppressed"
                        class="flex items-center justify-center py-3 text-center"
                    >
                        <p class="text-xs text-red-500">
                            This lead is suppressed — cannot send replies
                        </p>
                    </div>
                    <div v-else class="flex items-end gap-2">
                        <textarea
                            ...
                            class="flex-1 resize-none rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-blue-300 focus:ring-1 focus:ring-blue-200"
                        ...
                        <button
                            ...
                            class="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-40"
                        >
                            <Send class="h-4 w-4" />
                            Send
                        </button>
                    ...
                    <p class="mt-1 text-[10px] text-gray-400">
                        To: {{ leadContext?.email }} &middot; From:
                        {{ leadContext?.brand?.name || 'Omni OS' }}
                    </p>
                </div>
            </template>
        </div>
