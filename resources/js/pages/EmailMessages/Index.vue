...
function goToPage(page: number) {
    ...
    router.get(`/email-messages?${params.toString()}`, {
        preserveScroll: true,
        preserveState: true,
        replace: true,
    });
    ...
    return (
        {
            ...
            needs_content: 'bg-purple-100 text-purple-700',
...
<template>
    ...
    <div>
        <div class="flex items-center border-b border-gray-200 px-4 py-3">
            ...
            <span
                class="ml-2 rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-medium text-gray-500"
                >{{ messages.total }}</span
            >
        </div>
        ...
        <div class="flex items-center gap-2 border-b border-gray-100 px-4 py-2">
            ...
            <input
                class="max-w-[200px] flex-1 rounded border-gray-200 text-xs"
        ...
        >
            ...
            <p class="text-sm font-medium text-gray-600">No email messages</p>
        </div>
                ...
                <tbody>
                    ...
                    >
                        <td class="px-4 py-2.5 text-xs text-gray-800">
                            {{ m.lead?.company_name || 'Lead #' + m.lead_id }}
                        </td>
                        <td class="px-4 py-2.5 text-xs text-gray-500">
                            Step {{ m.sequence_step }}
                        </td>
                        <td
                            class="max-w-[250px] truncate px-4 py-2.5 text-xs text-gray-600"
                        >
                        ...
                        <td class="px-4 py-2.5">
                            <span
                                ...
                                :class="approvalColor(m.approval_status)"
                        ...
                        <td class="px-4 py-2.5">
                            <button
                                ...
                                @click="toggleBody(m.id)"
                            >
                                {{ showBody[m.id] ? 'Hide' : 'View' }}
                            </button>
                        </td>
                    </tr>
                    <tr
                        v-for="m in messages.data.filter((m) => showBody[m.id])"
                        :key="'body-' + m.id"
                    >
                        <td colspan="8" class="bg-gray-50/50 px-6 py-3">
                            <div
                                class="max-h-64 overflow-y-auto rounded border border-gray-200 bg-white p-3"
                            >
                                <p
                                    class="mb-2 text-xs font-semibold text-gray-800"
                                >
                                    {{ m.subject }}
                                </p>
                                <div
                                    class="font-sans text-xs leading-relaxed whitespace-pre-wrap text-gray-600"
                                >
                                    {{ m.body }}
                                </div>
                            </div>
