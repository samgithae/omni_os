...
const expandedDays = ref<Set<string>>(
    new Set(Object.keys(props.groupedEvents)),
);
...
const dayGroups = computed(() => {
    return Object.entries(props.groupedEvents).map(([day, events]) => ({
        ...
        hasDailyBrief: (events as ActivityEventData[]).some(
            (e) => e.event_type === 'daily_brief',
        ),
    }));
});
...
function sourceLabel(source: string): string {
    ...
    if (parts.length >= 2)
        return (
            parts[1]?.charAt(0).toUpperCase() + parts[1]?.slice(1) || 'System'
        );
    return source;
}
...
function getCommentCount(event: ActivityEventData): number {
    return (
        (event as any).comments_count ?? (event as any).comments?.length ?? 0
    );
}
...
async function checkForNewEvents() {
    try {
        const params = new URLSearchParams({
            since: String(latestKnownId.value),
        });
    ...
    <div class="pb-16">
        ...
        <Transition name="slide-down">
            ...
            >
                ...
                {{ newEventCount }} new
                {{ newEventCount === 1 ? 'event' : 'events' }} — click to load
            </button>
        </Transition>
        ...
        >
            ...
            <button
                ...
                class="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-medium transition-colors"
                :class="
                    activeBrand === brand.slug && !activeAgent
                        ? 'text-white'
                        : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                "
                :style="
                ...
                @click="
                    filterByBrand(
                        activeBrand === brand.slug ? null : brand.slug,
                    )
                "
            >
                <span
        ...
        <div
            ...
            class="flex flex-wrap items-center gap-1 border-b border-gray-100 bg-gray-50/50 px-4 py-1.5"
        >
            <span
                class="mr-1 text-[10px] font-medium tracking-wider text-gray-400 uppercase"
                >Agent</span
            >
            <button
        ...
        <div v-else class="divide-y divide-gray-100">
                ...
                <Transition name="fade">
                    <div v-if="expandedDays.has(group.day)">
                        ...
                        >
                            ...
                            >
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <div
                                            v-if="event.body"
                                            class="mt-1 text-xs leading-relaxed whitespace-pre-wrap text-gray-700"
                                        >
                                            {{ event.body }}
                                        </div>
                                    </div>
                            ...
                            <div
                                v-else
                                class="border-l-2 px-4 py-2 transition-colors hover:bg-gray-50/50"
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div
                                        v-if="event.agent"
                                        class="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-purple-100 text-[9px] font-bold text-purple-700"
                                        :title="event.agent.display_name"
                                    >
                                        {{
                                            agentInitials(
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <Transition name="fade">
                                            ...
                                            >
                                                <div
                                                    v-if="event.body"
                                                    class="rounded-md bg-gray-50 px-3 py-2 text-xs leading-relaxed whitespace-pre-wrap text-gray-600"
                                                >
                                                    {{ event.body }}
                                                </div>
                                                <div
                                                ...
                                                <div
                                                    class="flex gap-2 text-[10px] text-gray-400"
                                                >
                                                    <span
                                                        >Source:
                                                        {{ event.source }}</span
                                                    >
                                                    <span v-if="event.agent"
                                                        >· Agent:
                                                        {{
                                                            event.agent
                                                                .display_name
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            :title="
                                                getCommentCount(event) > 0
                                                    ? `${getCommentCount(event)} comments`
                                                    : 'Reply'
                                            "
                                        >
                                ...
                                <Transition name="fade">
                                    <div
                                        v-if="threadOpen(event.id)"
                                        class="mt-2 ml-10 border-l-2 border-gray-200 pl-4"
                                    >
                                        <!-- Thread header -->
                                        <div
                                        ...
                                        <div
                                            v-if="
                                                getComments(event).length ===
                                                    0 && !getCommentCount(event)
                                            "
                                            class="mb-2 text-[10px] text-gray-400"
                                        >
                                            No comments yet
                                        </div>
                                        <div
                                            ...
                                            :key="comment.id"
                                            class="mb-2 rounded-lg border border-gray-100 px-3 py-2"
                                        >
                                            <div
                                            ...
                                            >
                                                ...
                                                <span
                                                    v-if="
                                                        comment.is_instruction
                                                    "
                                                    class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[9px] font-medium"
                                                    :class="
                                                        comment.instruction_status ===
                                                        'pending'
                                                            ? 'bg-amber-100 text-amber-700'
                                                            : comment.instruction_status ===
                                                                'acknowledged'
                                                ...
                                                >
                                                    ...
                                                    {{
                                                        comment.instruction_status ===
                                                        'pending'
                                                            ? '→ sent to Hermes ● pending'
                                                            : comment.instruction_status
                                                    }}
                                                </span>
                                            </div>
                                            <div
                                                class="mt-1 text-xs leading-relaxed text-gray-700"
                                            >
                                                {{ comment.body }}
                                            </div>
                                            <div
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <pre
                                                        class="mt-1 rounded bg-gray-50 p-1.5 font-mono text-[9px]"
                                                        >{{
                                                            JSON.stringify(
                                                                comment.metadata,
                                                                null,
                                        ...
                                        <div class="mt-2 flex items-end gap-2">
                                            <div class="flex-1">
                                                <textarea
                                                    v-model="
                                                        commentTexts[event.id]
                                                    "
                                                    rows="2"
                                                    placeholder="Write a reply..."
                                                    class="w-full resize-none rounded-lg border border-gray-200 px-3 py-2 text-xs outline-none focus:border-blue-300 focus:ring-1 focus:ring-blue-200"
                                                    @keydown.meta.enter="
                                                ...
                                                ></textarea>
                                                <label
                                                    class="mt-1 flex cursor-pointer items-center gap-1.5 text-[10px] text-gray-400 hover:text-gray-600"
                                                >
                                                    <input
                                                        type="checkbox"
                                                        v-model="
            ...
            <div class="px-4 py-4 text-center">
                <button
                    class="rounded-md bg-white px-4 py-2 text-xs font-medium text-gray-600 ring-1 ring-gray-300 ring-inset hover:bg-gray-50 disabled:opacity-50"
