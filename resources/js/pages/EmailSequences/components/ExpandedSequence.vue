...
function stepStatusIcon(step: StepInfo): string {
    ...
    if (step.approval_status === 'approved' && step.send_status === 'queued')
        return '✓';
...
function approvalBadge(step: StepInfo): string {
    ...
    if (step.approval_status === 'needs_content')
...
function approveSingle(emailId: number) {
    router.post(
        `/email-sequences/${emailId}/approve`,
        {},
        {
            preserveScroll: true,
            preserveState: true,
...
function rejectSingle(emailId: number) {
    router.post(
        `/email-sequences/${emailId}/reject`,
        {},
        {
            preserveScroll: true,
            preserveState: true,
...
function approveAllPending() {
    ...
    router.post(
        '/email-sequences/approve',
        { lead_ids: [pendingIds[0]] },
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => emit('approved'),
...
const hasAnyPending = props.steps.some(
        ...
        <Transition name="expand" mode="out-in">
            <div class="space-y-4">
                ...
                >
                    <div class="flex items-start gap-2">
                        ...
                        <div class="text-sm text-amber-800">
                            ...
                            <p>
                                Steps [{{ missingSteps.join(', ') }}] have not
                                been generated yet. Hermes will fill these on
                                the next generation run (every 60 min). Approval
                                is blocked until all steps have content.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                >
                    ...
                    <div
                        v-if="step.exists"
                        class="absolute top-3 left-[7px] h-full w-px bg-gray-200"
                    />
                    <div
                        v-if="step.exists"
                        class="absolute top-[5px] left-0 h-[15px] w-[15px] rounded-full border-2"
                        :class="{
                            'border-green-500 bg-green-100':
                                step.clicked_at ||
                                step.opened_at ||
                                step.send_status === 'sent',
                            'border-amber-400 bg-amber-100':
                                step.approval_status === 'pending',
                            'border-purple-400 bg-purple-100':
                                step.approval_status === 'needs_content',
                    ...
                    <div :class="{ 'opacity-40': !step.exists }">
                        ...
                        <div class="mb-1 flex items-center justify-between">
                            ...
                            >
                                <span v-if="step.sent_at"
                                    >Sent {{ formatDate(step.sent_at) }}</span
                                >
                                <span v-if="step.opened_at"
                                    >Opened
                                    {{ formatDate(step.opened_at) }}</span
                                >
                                <span
                                    v-else-if="step.send_status === 'sent'"
                                    class="text-amber-500"
                                    >Not opened</span
                                >
                                <span v-if="step.clicked_at"
                                    >Clicked
                                    {{ formatDate(step.clicked_at) }}</span
                                >
                                <span
                                    v-else-if="step.opened_at"
                                    class="text-amber-500"
                                    >No clicks</span
                                >
                                <span
                                    v-if="step.scheduled_for && !step.sent_at"
                                >
                                    Scheduled:
                        ...
                        >
                            ...
                            <button
                                v-if="
                                    ...
                                    step.id
                                "
                                class="rounded bg-white px-2.5 py-0.5 text-[10px] font-medium text-gray-600 ring-1 ring-gray-300 ring-inset hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-40"
                                :disabled="sequenceComplete === false"
                        ...
                        <Transition name="fade">
                            <div
                                v-if="previewOpen[step.step] && step.exists"
                                class="mb-3"
                            >
                                <EmailPreview
                ...
                >
                    ...
                    <button
                        class="rounded bg-white px-3 py-1 text-xs font-medium text-gray-600 ring-1 ring-gray-300 ring-inset hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-40"
                        :disabled="sequenceComplete === false"
