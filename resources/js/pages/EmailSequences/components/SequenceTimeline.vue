...
function stateColors(state: string) {
  switch (state) {
    ...
    default: return 'bg-white border-dashed border-gray-300'
  }
}
...
function findCurrentStepIndex(): number {
  const idx = props.steps.findIndex(s => s.exists && s.approval_status === 'pending')
  if (idx >= 0) return idx
  ...
  <div class="flex items-center gap-0">
    ...
    >
      ...
      <div class="flex flex-col items-center">
        ...
        >
          <span
            v-if="stepState(step) !== 'empty' && stepState(step) !== 'draft'"
        ...
        <span
          class="mt-1 whitespace-nowrap text-[10px] leading-tight"
          :class="{
      ...
      <div
        ...
        class="mx-0.5 h-[2px] w-5 self-start mt-[11px]"
        :class="{
          'bg-green-300': stepState(step) === 'sent_opened',
          'bg-blue-300': stepState(step) === 'sent' || stepState(step) === 'approved',
          'bg-gray-200': stepState(step) !== 'sent_opened' && stepState(step) !== 'sent' && stepState(step) !== 'approved',
        }"
      />
    </div>
  </div>
